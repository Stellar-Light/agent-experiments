 ...
 * HIDDEN ON-CHAIN:
 *
 *   MOMO (seller) — a data vendor. Builds a signed market report from live
 *   StellarLight data and sells it for 40 XLM.
 *   PIP (buyer)  — wants the report. Pays the invoice CONFIDENTIALLY via
 *   OpenZeppelin Confidential Tokens (testnet deployment), then receives the
 *   signed goods after Momo verifies the credit by decrypting it.
 *
 ...
 * Momo's decrypted credit (what the recipient sees), Pip's decrypted change
...
const DEPOSIT = 100n * STROOP;   // Pip's public deposit into the confidential contract
const PRICE = 40n * STROOP;      // Momo's invoice — this number never appears on-chain
...
/** MOMO's product: a small market report built from live StellarLight data, signed. */
async function buildProduct(momoKp) {
  let facts;
  try {
  ...
  const signature = momoKp.sign(hash(payload)).toString("base64");
  return { body, sha256: hash(payload).toString("hex"), signature, signer: momoKp.publicKey() };
}
...
async function main() {
  ...
  const pip = Keypair.random();  // buyer
  const momo = Keypair.random();  // seller
  ...
  const pipSigner = keypairSigner(pip.secret(), NETWORK);
  const vegaSigner = keypairSigner(momo.secret(), NETWORK);
  receipt.agents = {
    pip: { role: "buyer", address: pip.publicKey() },
    momo: { role: "seller (data vendor)", address: momo.publicKey() },
  };
  console.log(`\n[0] PIP (buyer)  ${pip.publicKey()}\n    MOMO (seller) ${momo.publicKey()}`);
  await Promise.all([fundAccount(pip.publicKey()), fundAccount(momo.publicKey())]);
  console.log("    both funded via friendbot");

  // ── 1 · confidential identities derived, nothing stored
  const pipId = confidentialIdentity(pip);
  const momoId = confidentialIdentity(momo);
  ...
  for (const [name, id, signer] of [["pip", pipId, pipSigner], ["momo", momoId, vegaSigner]]) {
  ...
  say("pip", "Requesting product: latest settlement-currency brief. What's your price?");
  say("momo", `Quote: 40 XLM, payable confidentially to ${momo.publicKey().slice(0, 8)}… — amount will not appear on-chain.`, { priceStroops: PRICE.toString() });

  // Pip provisions confidential funds: public deposit (by design), then merge
  console.log("\n[5] pip provisions confidential balance…");
  const dep = await client.invoke(CONTRACTS.token, "deposit", [addr(pip.publicKey()), addr(pip.publicKey()), i128Val(DEPOSIT)], pipSigner);
  chainLog("deposit-pip", dep, "public 100 XLM enters the confidential contract (deposits are public by design)");
  const mrg = await client.invoke(CONTRACTS.token, "merge", [addr(pip.publicKey())], pipSigner);
  chainLog("merge-pip", mrg, "receiving → spendable");

  const pipState = await rebuildState(client, pip.publicKey(), pipId.keys);
  const spendable = pipState.state().spendable;
  console.log(`    pip spendable = ${spendable.v / STROOP} XLM (decrypted locally)`);

  // The confidential payment itself
  say("pip", "Paying the invoice confidentially now.");
  const t0 = Date.now();
  const transfer = await proveTransfer({
    keys: pipId.keys, v: spendable.v, r: spendable.r, amount: PRICE,
    pvkB: momoId.keys.PVK, kAudR: kAud, kAudS: kAud,
  });
  ...
  const pay = await client.invoke(
    ...
    [addr(pip.publicKey()), addr(momo.publicKey()), bytesVal(transfer.payload)], pipSigner,
  );
  ...
  say("pip", `Payment sent. tx ${pay.hash.slice(0, 12)}… — check your receiving balance.`);

  // ── 6 · momo verifies the credit by DECRYPTING it, then delivers
  console.log("\n[6] momo verifies payment by rebuilding + decrypting its own state…");
  const momoEngine = await rebuildState(client, momo.publicKey(), momoId.keys);
  const credited = momoEngine.receiving();
  console.log(`    momo decrypted receiving = ${credited.v / STROOP} XLM`);
  if (credited.v !== PRICE) throw new Error(`momo expected ${PRICE}, decrypted ${credited.v}`);
  say("momo", `Payment verified: decrypted exactly 40 XLM received. Delivering goods.`);
  receipt.product = await buildProduct(momo);
  say("momo", `Delivered: "${receipt.product.body.title}" — sha256 ${receipt.product.sha256.slice(0, 16)}…, signed.`);
  const sigOk = momo.verify(hash(Buffer.from(JSON.stringify(receipt.product.body))), Buffer.from(receipt.product.signature, "base64"));
  say("pip", `Goods received; signature ${sigOk ? "verified" : "FAILED"}. Transaction complete.`);
  if (!sigOk) throw new Error("product signature failed verification");

  // ── 7 · the four views
  console.log("\n[7] assembling the four views…");
  const momoOnchain = await client.confidentialBalance(momo.publicKey());
  const pipOnchain = await client.confidentialBalance(pip.publicKey());
  const pipAfter = await rebuildState(client, pip.publicKey(), pipId.keys);
  const pipCheck = pipAfter.verifyAgainstChain({
    spendableC: pointToBytes(pipOnchain.spendableBalance),
    receivingC: pointToBytes(pipOnchain.receivingBalance),
  });
  receipt.views = {
    public_chain: {
      what: "what any explorer or observer sees",
      vega_receiving_commitment: hex(pointToBytes(momoOnchain.receivingBalance)),
      nova_spendable_commitment: hex(pointToBytes(pipOnchain.spendableBalance)),
    ...
    sender_nova: {
      ...
      decrypted_spendable_stroops: pipAfter.state().spendable.v.toString(),
      decrypted_spendable_xlm: (pipAfter.state().spendable.v / STROOP).toString(),
      verified_against_chain: pipCheck.ok === true,
    },
    auditor: {
  ...
  if (publishSession) {
    ...
    writeFileSync(new URL("./session.json", import.meta.url).pathname, JSON.stringify({
      ...
      pip: { address: pip.publicKey(), secret: pip.secret() },
      momo: { address: momo.publicKey(), secret: momo.secret() },
